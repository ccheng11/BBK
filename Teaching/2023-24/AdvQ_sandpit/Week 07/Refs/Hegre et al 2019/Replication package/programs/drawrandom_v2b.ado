program drawrandom_v2b, rclass
	/*Define the syntax*/ 
	
	syntax varlist [if], depvar(varname) random(varname) drawyear(real) draws(real) saveto(string) ///
		esttabpath(string) fyear(int) lyear(int)
	*****************************************************************************
	...
	*summarize `varlist'
	...
	forvalues s = `depmin'(1)`depmax' {
		...
		/*Get start values */
		/*
		quietly logit inc `varlist'
		matrix start = e(b)
		*/
		/*run the multilevel logistic model */
		noisily melogit inc `varlist' if year >= `fyear' & year <= `lyear' || `random':, startvalues(zero) iterate(100) 
		...
		esttab using `esttabpath'_`s'_o,  tex wide nogaps replace 
		...
		/*countries with missing re mean and sds, islands, get overall mean */
		display "NOTE: countries with missing re mean and sds, islands, get overall mean" 
		svmat means
		egen mean = mean(means1)
		replace means1 = mean if means1==.
		mkmat means1, matrix(means)
		clear
		svmat sds
		egen mean = mean(sds1)
		replace sds1 = mean if sds1==.
		mkmat sds1, matrix(sds)
		...
		group specific random effects distribution. Add everything to the matrix defined above */
		 forvalues i = 1(1)`countries' {
			 forvalues j = 4(1)`tempdraws' {
				display "drawing country country: `i'"
				 quietly capture drop random
				 matrix tempmean = means[`i',1]
				matrix tempse = sds[`i',1]
				 quietly drawnorm random, n(1) means(tempmean) sds(tempse) clear
				 mkmat random, matrix(tempreal)
				 matrix randomeffects[`i',`j'] =  tempreal
				quietly  drop random
			}
		}
