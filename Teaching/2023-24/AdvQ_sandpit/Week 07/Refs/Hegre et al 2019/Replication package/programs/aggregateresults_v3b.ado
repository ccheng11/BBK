		...
		if _rc==0 {
			foreach summtype in "m" "v" {
				...
				*drop if year == `sim_lyear'
				...
				/* Expanding for regional and global aggregates */
				local observations = _N
				local newobs = _N + (10 * `yearnos')
				set obs `newobs'
				forvalues reg = 1(1)9 {
					local thisregno = 1000 + `reg'
					quietly replace gwno = `thisregno' if ///
						_n > (`observations' + ((`reg'-1)*`yearnos')) & _n <= (`observations' + ((`reg')*`yearnos')) 
					forvalues y =`fyear'(1)`lyear' {
						quietly replace year = `y' if _n == (`observations' + ((`reg'-1)*`yearnos') + (`y' - `fyear' + 1))
					}
				}
				quietly replace gwno = 1100 if _n > (`observations' + (9*`yearnos')) ///
					& _n <= (`observations' + (10*`yearnos')) 
				quietly forvalues y =`fyear'(1)`lyear' {
					replace year = `y' if _n == (`observations' + (9*`yearnos') + (`y' - `fyear' + 1))
				}
...
*						display "dvlb " `dvlb'
					...
					local sims = r(max)
					local observations = _N
					local newobs = _N + (10 * `yearnos' * `sims')
					set obs `newobs'
					local proc_obs = `observations'
					forvalues sim = 1(1)`sims' {
						quietly replace simno = `sim' if _n > `proc_obs' & _n <= (`proc_obs' + (10 * `yearnos'))
						forvalues reg = 1(1)9 {
							local thisregno = 1000 + `reg'
							quietly replace gwno = `thisregno' if ///
								_n > (`proc_obs' + ((`reg'-1)*`yearnos')) & _n <= (`proc_obs' + ((`reg')*`yearnos')) 
							quietly forvalues y =`fyear'(1)`lyear' {
								replace year = `y' if _n == (`proc_obs' + ((`reg'-1)*`yearnos') + (`y' - `fyear' + 1))
							}
						}
						quietly replace gwno = 1100 if _n > (`proc_obs' + (9*`yearnos')) ///
							& _n <= (`proc_obs' + (10*`yearnos')) 
						forvalues y =`fyear'(1)`lyear' {
							quietly replace year = `y' if _n == (`proc_obs' + (9*`yearnos') + (`y' - `fyear' + 1))
						}
						local proc_obs = `proc_obs' +  (10 * `yearnos')
					}
					capture drop _merge
					...
					foreach var in sim_0 sim_1 sim_2 phat0 logit0_o phat1 logit1_o phat2 logit2_o { /* Needs generalization */
