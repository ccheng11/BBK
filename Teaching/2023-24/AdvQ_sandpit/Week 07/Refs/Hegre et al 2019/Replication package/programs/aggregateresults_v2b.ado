...
*						display "dvlb " `dvlb'
					...
					keep simno gwno year `depvar' `summarize' phat* logit* cutoff*
					quietly save Res_`simtype'_`i'_`j'_`k'_`s'.dta, replace
					/* Collapsing it */
					collapse (count) n=simno (mean) logit* `summarize', by(`aggregationtext') fast
					quietly summ n
					local this_n = r(mean)
...
forvalues i = 1(1)`ln1' {
	...
	*/				
			...
			if `agg_n' == 1 {
				...
				matrix temp = aggv[1..`mrows',4..`mcols'] / 1 /* If `agg_n' == 1, variance is 0 and the discounting is irrelevant */
			}
