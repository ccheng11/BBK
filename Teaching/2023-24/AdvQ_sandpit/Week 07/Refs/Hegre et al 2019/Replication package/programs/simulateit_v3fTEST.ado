program simulateit_v3fTEST
    ...
    syntax, simset(namelist) workdir(string) depvar(string) depvarvalues(int) ///
		...
		numpars(numlist min=4 max=4 integer) years(numlist min=4 max=5 integer) parafilename(string) resultsfile(string) ///
		logfile(string) simresdir(string) zeroline(int) endoeq(string) endoposition(int) endodraws(int) ///
		project(string) seedno(int) addyears(int) reterms(int)
...
local sim_fyear = word("`years'", 4)
*local endo_lyear = word("`years'", 5)

local endo_lyear = `est_lyear'
	
local simyears = `sim_lyear' - `sim_fyear' 
...
foreach model of local modellist {
	if "`project'" == "RegimePredictions" & "`simtype'" == "oos" & "`simsetname'" == "RPoosv9" {
		if `model' == 1 {
		local parafilename = "parameters_baseline_RP_RE1.txt" 
		}
		if `model' == 2 {
		local parafilename = "parameters_baseline_RP_RE2.txt" 
		}
		if `model' == 3 {
		local parafilename = "parameters_baseline_RP_RE3.txt" 
		}
		if `model' == 4 {
		local parafilename = "parameters_baseline_RP_RE4.txt" 
		}
		if `model' == 5 {
		local parafilename = "parameters_baseline_RP_RE5.txt" 
		}	
		if `model' == 6 {
		local parafilename = "parameters_baseline_RP_RE6.txt" 
		}	
		if `model' == 7 {
		local parafilename = "parameters_baseline_RP_RE7.txt" 
		}	
		display "Original parameter file: `pfnorig'"
		display "Endogenous equation = `endogenous_eq'"
	}
...
*			local parafilename = subinstr("`pfnorig'",".","`mlab'.",1) 
		...
		/* CR project addendum: rename parameter baseline to match model*/		
		if "`project'" == "ConflictRegimePredictions" {
		if `model' >= 7 {
		local parafilename = "parameters_baseline_CR_lts.txt" 
		}
		display "Original parameter file: `pfnorig'"
		display "Now using parameter file: `parafilename'"
		display "Endogenous equation = `endogenous_eq'"
	}
		...
		display "Endogenous equation: `endoeq'"
		sleep 3000
		display "Input dataset: `indata'"
		use "`indata'", clear
		...
		if wordcount("`endoeq'") > 1 { /* If an endogenous equation has been specified */
			...
			*quietly 
			endogenousgrowth_v2 `endoeq', lyear(`endo_lyear') draws(`endodraws') simset(`simset') scenario(`scenario')
			esttab using "`workdir'/Results/FEmodel_`simset'_`scenario'.tex", replace unstack fragment wide scalars(ll) ///
				drop(_Igwno*) title("Fixed-effects model of effect of conflict on growth; 19xx-`est_lyear'") 
		}
		...
		*else {
		*quietly 
		drawrandom_v2b `indeps', random(gwno) depvar(`depvar') drawyear(`est_lyear') esttabpath("Results/`simset'/re_model_`model'") ///
					   draws(`redraws') saveto("Results/`simset'/re_model_`model'") fyear(`est_fyear') lyear(`est_lyear')
		*	}
...
*				quietly include "Scripts/Data_manipulation_Temp.do"
				...
				*/
				...
				if "`depvar'" == "confreg"  { 				
					qui merge m:1 gwno using "Results/`simset'/re_model_`model'_0_o.dta", gen(remerge) keepusing(random`r'_0_o)
					rename random`r' random_0
					keep if remerge == 3
					capture drop remerge
					...
					keep if remerge == 3
					capture drop remerge
					qui merge m:1 gwno using "Results/`simset'/re_model_`model'_3_o.dta", gen(remerge) keepusing(random`r'_3_o)
					rename random`r' random_3
					keep if remerge == 3
					capture drop remerge
					qui merge m:1 gwno using "Results/`simset'/re_model_`model'_4_o.dta", gen(remerge) keepusing(random`r'_4_o)
					rename random`r' random_4
					keep if remerge == 3
					capture drop remerge
					qui merge m:1 gwno using "Results/`simset'/re_model_`model'_5_o.dta", gen(remerge) keepusing(random`r'_5_o)
					rename random`r' random_5
					keep if remerge == 3
					capture drop remerge
				}
				...
				/*ADD YEARS TO GET AROUND Y2K PROBLEM */
				local lastactualyear = `sim_lyear' - `addyears'
				display "last actual year of simulation is: `lastactualyear'"
				drop if year > `lastactualyear'
				replace year = year + `addyears'
				/*END ADD YEARS */
				/* end random addendum */	
				*/
					quietly compress
					...
					if wordcount("`endoeq'") > 1 { /* If an endogenous equation has been specified */
						...
						*matrix list thesecoefs
						...
						if "`project'" == "ImpactCorrectedSSP" {
							local endoparastatement = "SUPPVAR; gdpoffset; lv(gdpoffset) + "
						}
...
*							}
						...
						*/
						if "`project'" != "ImpactCorrectedSSP" {
							...
							local endoparastatement = "`endoparastatement'" + "0"
						}
...
*stop					
					...
					*	local temp = `zeroline' + `l'
					*	display "Editing line number: `temp'"
					...
					*display "here"
					...
					quietly replace v1 = "STARTTIME; `sim_fyear'" if _n == `zeroline' + `maxparas' + `re_terms' + 6
					...
					*quietly 
					set seed `seedno'
					local betaline = 1
					Simulator_v2 "`parameter_outfile'" `betaline'
