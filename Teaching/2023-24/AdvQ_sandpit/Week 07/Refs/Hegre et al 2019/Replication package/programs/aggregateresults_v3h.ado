...
*						display "dvlb " `dvlb'
		...
		preserve
		quietly replace year = year - `addyears'
		local outdata = "`currdir'" + "/Results/" + "`simsetname'" + "/" + "Aggmeans_`simtype'_`i'_`j'_x_x_YC.dta"
		display "Saving aggregated means to: `outdata'"
		quietly save "`outdata'", replace
		restore
		...
		forvalues k = 1(1)`ln3' {
			...
			if _rc==0 {
				...
				merge m:1 gwno year using `indata'
