...
*			local parafilename = subinstr("`pfnorig'",".","`mlab'.",1) 
		...
		*else {
		*quietly 
		drawrandom_v2 `indeps', random(gwno) depvar(`depvar') drawyear(2019) esttabpath("Results/`simset'/re_model_`model'") ///
...
*stop					
					...
					*display "here"
					...
					/* quietly replace v1 = "IFS; if year >= `est_fyear' & year <= `est_lyear'" if _n == `zeroline' + `maxparas' + 2
					quietly replace v1 = "BETADRAWS; 1" if _n == `zeroline' + `maxparas' +  3
					quietly replace v1 = "SPLIT; none" if _n == `zeroline' + `maxparas'  + 4
					quietly replace v1 = "ITERATIONS; `simyears'" if _n == `zeroline' + `maxparas'  + 5
					quietly replace v1 = "STARTTIME; `sim_fyear'" if _n == `zeroline' + `maxparas'  + 6
					quietly replace v1 = "LOGFILE; `logfile'.txt" if _n == `zeroline' + `maxparas'  + 7
					quietly replace v1 = "SIMULATIONS; `simnos'" if _n == `zeroline' + `maxparas'  + 8
					quietly replace v1 = "ENDPARAMS; baseoutcome(0)" if _n == `zeroline' + `maxparas'  + 9
					quietly replace v1 = "RESULTFILE; `simresdir'/Res_`simtype'_`scenario'_`mlab'_`r'_`b'.txt" ///
						if _n == `zeroline' + `maxparas'  + 10 */
