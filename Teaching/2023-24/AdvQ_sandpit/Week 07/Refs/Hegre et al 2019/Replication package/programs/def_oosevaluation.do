capture program drop oosevaluation

program oosevaluation
	version 12
    syntax namelist, simsetname(namelist) levels(numlist min=3 max=3 integer) compfile(string)
	/* 
	...
	*/
	...
	dir
		...
		forvalues j = 1(1)`ln2' { /* Index for model number */
			...
			forvalues k = 1(1)`ln3' {
				display "k: " `k'
				capture confirm file "c_Res_oos_comp_`i'_`j'_`k'.dta"
				if _rc==0 {
