program oosevaluation_v2
	version 12
    syntax namelist, simsetname(namelist) depvar(string) depvarvalues(int) levels(numlist min=2 max=2 integer) compfile(string)
	...
	*/
	...
	cd `path'
*	dir
		forvalues i = 1(1)`ln1' { /* Index for scenario number */
		display "i: " `i'
		forvalues j = 1(1)`ln2' { /* Index for model number */
		display "j: " `j'
			capture confirm file "Aggregated_c_oos_`i'_`j'_x_x.dta"
			if _rc==0 {
				use "Aggregated_c_oos_`i'_`j'_x_x.dta", replace

				keep gwno year m_phat1 m_phat2 /* TO DO : GENERALIZE TO OTHER NUMBER OF VALUES */
				matrix ROC[1,`j'] = `j'
				summ year
				local fyear = r(min)
				local lyear = r(max)
				...
				merge 1:1 gwno year using "../../InputData/`compfile'", keepusing(`depvar') gen(origmerge)
				keep if origmerge == 3
				...
				drop origmerge
				gen sim = m_phat1 + m_phat2 /* TO DO : GENERALIZE TO OTHER NUMBER OF VALUES */
				gen inc = `depvar'
				recode inc 2=1
				...
				display "The file Aggregated_c_oos_`i'_`j'_x_x.dta does not exist"
			} /* end else */
...
outtable using OOSresults.tex, mat(oosresults) replace caption("Results of OOS evaluation, simulation set `simsetname'") format(%9.3f )
