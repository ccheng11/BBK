program oosevaluation_v3f
	version 11
    syntax namelist, simsetname(namelist) depmin(int) depmax(int) depvar(string) depvarvalues(int) ///
		levels(numlist min=2 max=2 integer) compfile(string) models(numlist) i10width(int)
	...
	*/
	...
	local i10width = `i10width' /* How many years at the end of the sample to include in i10 evaluation */
...
*	forvalues s = 2(1)`depmax' { /* Looping over all values for outcome variable */
	forvalues s = `depmin'(1)`depmax' { /* Looping over all values for outcome variable */
		matrix define BrierScores_`s' = J(`ln2',5,0)
		matrix colnames BrierScores_`s' = Model inc ons trm i10
		matrix define AUCScores_`s' = J(`ln2',5,0)
		matrix colnames AUCScores_`s' = Model inc ons trm i10
		forvalues scenario = 1(1)`ln1' { /* Index for scenario number */
			...
			foreach model of local models { /* Index for model number */
				...
				if _rc==0 {
					...
					if `s' == 0 & `scenario' == 1 {
					preserve 
						keep gwno year m_phat* `depvar'
						save oos_manual_`model'
					restore
					}
					...
					foreach obs in inc_`s' ons_`s' trm_`s' i10_`s' { 
						...
						*sleep 1000
						...
						matrix AUCScores_`s'[`model',1] = `model'
						matrix AUCScores_`s'[`model',`fpos_simple'+1] = r(area)
*						if "`obs'" == "i10_`s'" {
*							stop
*						}
						local ptype = 0
						foreach p in 0.005 0.01 0.025 0.05 0.075 0.10 0.20 0.35 0.50 0.75 1.00 {
							...
							*local ptype = `ptype' + 1
							...
							matrix BrierScores_`s'[`model',1] = `model'
							matrix BrierScores_`s'[`model',`fpos_simple'+1] = r(brier)
							local ptype = `ptype' + 1
					...
					if `s' == `depmin' { /* Only done once for each scenario and model */
						...
						if `depmax'-`depmin' == 5 {
							gen temp = max(m_phat0, m_phat1, m_phat2, m_phat3, m_phat4, m_phat5) /* NOTE: needs generalization */
							label define outcome 0 "AP" 1 "AC" 2 "SP" 3 "SC" 4 "DP" 5 "DC"
					}
...
*/
	...
	matrix Brier = (Brier0 + Brier1 + Brier2 + Brier3 + Brier4 + Brier5) / 6
	matrix AUC = (AUC0 + AUC1 + AUC2 + AUC3 + AUC4 + AUC5)/6
