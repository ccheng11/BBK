...
*				} /* end if `summtype' == "m" */
			...
			display "Moving to next results file"
			local j = `j' + 1 
		} /* end else */
		matrix dir
...
*					forvalues num = 1(1)`yearnos'{
					forvalues num = 2(1)`yearnos'{
						...
						local cmrows =  colsof(currentmeans)
						
						matrix listcurrentm = currentmeans[1..`cmrows',1..`cmrows']
		...
		forvalues k = 1(1)`ln3' {
			...
			*		matrix listcurrentv = currentvars[1..9,1..9]
					matrix listcurrentv = currentvars[1..15,1..15]
					matrix ns = J(`mrows',1,`agg_n') /* Matrix to hold n temporarily */
