program aggregateresults_v4b
	version 12
    syntax namelist, simsetname(namelist) depvar(string) depvarvalues(int) simtype(namelist) ///
		COLLapseover(int) levels(numlist min=4 max=4 integer) path(string) fyear(int) lyear(int) addyears(int) /// 
		[summarize(namelist)] 
...
forvalues i = 1(1)`ln1' { /* Scenario level */
	forvalues j = 1(1)`ln2' { /* Model level */
...
*				} /* end if `summtype' == "m" */
				forvalues dvlb = `depmin'(1)`depmax' {
					...
					quietly gen diffp`dvlb' = 0 /* To hold difference from scenario 1 */
				}
				forvalues dvlb = `depmin'(1)`depmax' {
					quietly gen logit`dvlb'_o = 0
					quietly gen difflogit`dvlb'_o = 0 /* To hold difference from scenario 1 */
				}
				forvalues dvlb = `depmin'(1)`depmax' {
					local next = `dvlb' + 1
...
*					display "Here 1"
					...
					quietly gen diffp0 = 0
					quietly gen logit0_o = ln(phat0/(1-phat0))
					quietly gen difflogit0_o = 0
...
*						display "dvlb " `dvlb'
						...
						quietly gen diffp`dvlb' = 0
						quietly gen logit`dvlb'_o = ln(phat`dvlb'/(1-phat`dvlb'))
						quietly gen difflogit`dvlb' = 0
					} /* end foreach dvlb in `depvarvalues'  */
					forvalues dvlb = `depmin'(1)`depmax' {
						local next = `dvlb' + 1
						forvalues dvlc = `next'(1)`depmax' { /* Prepares for aggregating logits for all pairwise comparisons */
					...
					keep simno gwno year `depvar' sim_* `summarize' phat* logit* cutoff* diff*
