program simulateit_v3fTEST
    ...
    syntax, simset(namelist) workdir(string) depvar(string) depvarvalues(int) ///
		...
		project(string) seedno(int) addyears(int)
...
foreach model of local modellist {
	if "`project'" == "ConflictTrap" & "`simtype'" == "oos" {
		if `model' == 1 {
		local parafilename = "parameters_baseline_CTM1.txt" 
		}
		if `model' == 2 {
		local parafilename = "parameters_baseline_CTM2.txt" 
		}
		if `model' == 3 {
		local parafilename = "parameters_baseline_CTM3.txt" 
		}
		if `model' == 4 {
		local parafilename = "parameters_baseline_CTM4.txt" 
		}
		if `model' == 5 {
		local parafilename = "parameters_baseline_CTM5.txt" 
		}	
		if `model' == 6 {
		local parafilename = "parameters_baseline_CTM6.txt" 
		}	
		if `model' == 7 {
		local parafilename = "parameters_baseline_CTM7.txt" 
		}	
...
*			local parafilename = subinstr("`pfnorig'",".","`mlab'.",1) 
		...
		*else {
		*quietly 
		drawrandom_v2 `indeps', random(gwno) depvar(`depvar') drawyear(`est_lyear') esttabpath("Results/`simset'/re_model_`model'") ///
...
*				quietly include "Scripts/Data_manipulation_Temp.do"
				...
				*/
				*else {
					qui merge m:1 gwno using "Results/`simset'/re_model_`model'_0_o.dta", gen(remerge) keepusing(random`r'_1_o)
