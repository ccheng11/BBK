...
program simulateit
	version 12
    syntax, simset(namelist) workdir(string) depvar(string) inputdatastub(string) mmatrix(string) modellist(numlist integer) simtype(string) ///
		numpars(numlist min=4 max=4 integer) years(numlist min=3 max=3 integer) parafilename(string) resultsfile(string) ///
		logfile(string) simresdir(string)
