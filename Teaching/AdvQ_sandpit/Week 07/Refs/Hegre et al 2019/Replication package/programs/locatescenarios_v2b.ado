capture program drop locatescenarios_v2b

program locatescenarios_v2b
	version 12
...
/* MUST DEFINE ANOTHER BASELINE SCENARIO FOR WAR/CONFLICT COUTNRIES; MAKE SURE THEY ARE IN WAR */
						...
						if "`scenariostyle'" == "group" {
							...
							replace group = 1 if gwno == 520 | gwno == 652 | gwno == 678 | gwno == 700 ///
								| gwno == 770 | gwno == 475 |  gwno == 645
							...
							*/
 							...
 							replace scenario =  2 if sc_cnt == 2 & group == 1 /* Deescalation in middle/poor war countries Gr. 1 */
							
							replace scenario =  4  if sc_cnt == 1 & group == 1  /* Peace in poor war countries Gr. 1  */
								
							replace scenario =  3 if  sc_cnt == 3 & group == 1 /* Continued Major conflict in poor war countries Gr. 1  */
