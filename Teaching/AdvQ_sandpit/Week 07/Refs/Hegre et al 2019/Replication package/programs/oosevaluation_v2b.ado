program oosevaluation_v2b
	version 12
    syntax namelist, simsetname(namelist) depmin(int) depmax(int) depvar(string) depvarvalues(int) levels(numlist min=2 max=2 integer) compfile(string)
		...
		forvalues s = `depmin'(1)`depmax' {
			...
			display "j: " `j'
				...
				if _rc==0 {
					...
					display "Currently checking regime state" `s'
					...
					foreach obs in inc ons trm i10 {
						...
						foreach p in 0.005 0.01 0.015 0.025 0.05 0.075 0.10 0.20 0.35 0.50 0.75 {
							...
							display " "
							display "**************************************************************"
							display "******** Comparing given estimation model: " `j' ", outcome " `s' " ********"
