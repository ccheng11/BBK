program aggregateresults_v3CT
    ...
    syntax namelist, simsetname(namelist) depvar(string) depvarvalues(int) simtype(namelist) ///
		...
		/*
		Legg til suppvar ikkelogget bnpcap - også i masterdofil
		Sjekk at aggregateresultsv3g tar med summarize variable for gwno > 1000
	...
	*/
	...
	local regnos = 12 /* Number of artificial countries corresponding to regions; excluding 'globally' */
...
forvalues i = 1(1)`ln1' { 
	forvalues j = 1(1)`ln2' {
		...
		display "************************************************************************"
		capture confirm file "Res_`simtype'_L_1_1_1_1.txt"
		if _rc==0 {
				display "Preparing aggregation matrix agg`summtype': Reading in file Res_`simtype'_L_1_1_1_1.txt... (namelist: `namelist')"
				insheet using "Res_`simtype'_L_1_1_1_1.txt", names clear
				sum simno
				local simnomin = r(min)
				keep if simno == `simnomin'
...
*				} /* end if `summtype' == "m" */
			...
			display "The file Res_`simtype'_L_1_1_1_1.txt does not exist"
		} /* end else */
		matrix dir
...
*		}
		...
		forvalues k = 1(1)`ln3' {
			forvalues s= 1(1)`ln4' {
				capture confirm file "Res_`simtype'_L_`i'_`j'_`k'_`s'.txt"
				if _rc==0 {
					display "First pass. Reading in file Res_`simtype'_L_`i'_`j'_`k'_`s'.txt... (namelist: `namelist')"
					quietly insheet using "Res_`simtype'_L_`i'_`j'_`k'_`s'.txt", names clear
					*drop if year == `sim_lyear'
*					display "Here 1"
...
*						display "dvlb " `dvlb'
					...
					capture drop _merge
					quietly merge m:1 gwno using RegionsCT.dta, keepusing(region)
					drop _merge
					forvalues reg = 1(1)`regnos' {
							quietly replace region = `reg' if gwno == `reg' + 1000
					}
					
					foreach var in sim_0 sim_1 sim_2 phat0 phat1 phat2 `summarize' { /* (removed lgdpcap) Needs generalization */
					...
					*stop
					...
					display "The file Res_`simtype'_L_`i'_`j'_`k'_`s'.txt does not exist"
				} /* end else */
