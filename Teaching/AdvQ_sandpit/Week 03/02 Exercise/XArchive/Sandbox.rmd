...
Use `?glm` to see more information about `glm()`. What does the argument `family` do? What is the default for the option `family`? 

**Question 2**

In addition to the predictors we have considered, what else should we consider and include in our logit regression model? 

\begin{itemize}
  \item Read through the codebook provided by Beesley and Cooper and find one variable and add it to the specified model.
  \item Interpret the odds ratio of the chosen predictor.
  \item Use predicted probability to evaluate the marginal effect of the chosen predictor.
  \item Calculate the mean squared error (MSE) to see if the new model with the chosen predictor performs better than the model without it.
\end{itemize}
...
The most intuitive way to use the **`predict()`** function. 
